...
describe("Test Transfer's fire functions", () => {
  ...
  test("test fireChange to target", () => {
    ...
    const { container } = render(<Transfer dataSource={dataSource} selectedKeys={['a']} targetKeys={['b', 'c']} onChange={handleChange}/>);
  ...
  test("test fireChange to source", () => {
    const handleChange = jest.fn();
    const { container } = render(<Transfer dataSource={dataSource} selectedKeys={['b', 'c']} targetKeys={['b', 'c']} onChange={handleChange}/>);
  ...
  test("test fireScroll", () => {
    ...
    const { container } = render(<Transfer listStyle={{height: 30}} dataSource={dataSource} onScroll={handleScroll}/>)
