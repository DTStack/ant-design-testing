...
export function fireCancelButton(container: IContainer) {
    const selector = `.${getProvider('prefixCls')}-modal-footer button.${getProvider('prefixCls')}-btn`;
...
export function fireCancel(container: IContainer, opt: { closeByButton: boolean } = { closeByButton: true }) {
